(function () {
    'use strict';
    angular.module('app')
        .controller('reviewDetailCtrl', ['$state','$stateParams', 'authService', 'EncApiService','globalFactory','DTOptionsBuilder', 'DTColumnDefBuilder', function ($state,$stateParams, authService, EncApiService,globalFactory,DTOptionsBuilder, DTColumnDefBuilder) {

            var vm = this;
            $("#global-progress-display").show();
            if (globalFactory.previousRoute !== '')
                localStorage.setItem('previousRoute', globalFactory.previousRoute);
            // vm.department = localStorage.getItem('deptId');
            ...
            EncApiService('getEvaluationCycleListForSelfEvaluation').then(function(response) {
